...
- **WTL-style message maps** — `BEGIN_EVENT_MAP` / `EVENT_HANDLER` /
  `END_EVENT_MAP` macros for concise event wiring.
...
| `proxy_impl.hpp` | CRTP `CProxyImpl<Derived, Traits>` with `_SetProxy`, `_Marshal`, `_MarshalNew` |
| `resource_impl.hpp` | Server-side `CResourceImpl<Derived, Traits>` |
| `event_map.hpp` | `CEventMap` base + `BEGIN_EVENT_MAP` / `EVENT_HANDLER` macros |
