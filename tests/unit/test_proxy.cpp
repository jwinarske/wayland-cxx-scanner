...
TEST(WlPtr, SwapWorks) {
    ...
    a.Swap(b);
    EXPECT_EQ(a->GetProxy(), reinterpret_cast<wl_proxy*>(static_cast<uintptr_t>(0x2222)));
    EXPECT_EQ(b->GetProxy(), reinterpret_cast<wl_proxy*>(static_cast<uintptr_t>(0x1111)));
    a->Detach();
    b->Detach();
}
