...
/// Abstract base class for objects that can process Wayland events (client-side)
/// or requests (server-side) by opcode dispatch.
class CEventMap {
public:
...
#define EVENT_HANDLER(opcode, func)                                            \
    if (_opcode == (opcode)) {                                                 \
        _EventMapClass::_CrackEvent_##opcode(                                  \
            static_cast<_EventMapClass*>(this), _args, &_EventMapClass::func); \
        return true;                                                           \
    }
...
#define BEGIN_REQUEST_MAP(thisClass)                                                  \
    ...
    bool ProcessRequest(                                                              \
        uint32_t _opcode, wl_client* _client, wl_resource* _resource, void** _args) { \
        (void)_args;

#define REQUEST_HANDLER(opcode, func)                                                  \
    if (_opcode == (opcode)) {                                                         \
        _RequestMapClass::_CrackRequest_##opcode(static_cast<_RequestMapClass*>(this), \
                                                 _client,                              \
                                                 _resource,                            \
                                                 _args,                                \
